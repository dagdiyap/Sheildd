...
def run_exp(replacement_method, num_poisoned_workers, KWARGS, client_selection_strategy, idx):
    ...
    file_rename = results_files[0].split(".")[0] + "_" + str(replacement_method).split(" ")[1] + ".csv"
    file_rename2 = worker_selections_files[0].split(".")[0] + "_" + str(replacement_method).split(" ")[1] + ".csv"
    save_results(results, file_rename)
    save_results(worker_selection, file_rename2)
